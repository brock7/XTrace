...
ulong Disasm(char *src,ulong srcsize,ulong srcip,
  ...
  while (size>0) {
    ...
    if (mode>=DISASM_FILE)
      ndump+=sprintf(da->dump+ndump,"%02X:",(ulong )(uchar )*cmd);
    da->nprefix++;
    cmd++; srcip++; size--; u++; };
  ...
  if (pd->mask==0) {                   // Command not found
    ...
    for (operand=0; operand<3; operand++) {
      ...
      if (operand==0) arg=(int)(uchar)pd->arg1;
      else if (operand==1) arg=(int)(uchar)pd->arg2;
      else arg=(int)(uchar)pd->arg3;
  ...
  if (da->error!=0) {                  // Hard error in command detected
    ...
    if (da->error==DAE_BADCMD &&
      ((uchar )*cmd==0x0F || (uchar )*cmd==0xFF) && size>0
    ) {
      if (mode>=DISASM_FILE) ndump+=sprintf(da->dump+ndump,"%02X", (ulong )(uchar )*cmd);
      cmd++; size--; };
    if (size>0) {
      if (mode>=DISASM_FILE) ndump+=sprintf(da->dump+ndump,"%02X", (ulong )(uchar )*cmd);
      cmd++; size--;
    ...
    if (mode>=DISASM_FILE) {
      ndump+=sprintf(da->dump+ndump,"%02X", (ulong )(uchar )*cmd++);
      if (hasrm) ndump+=sprintf(da->dump+ndump,"%02X", (ulong )(uchar )*cmd++);
      if (hassib) ndump+=sprintf(da->dump+ndump,"%02X", (ulong )(uchar )*cmd++);
      if (dispsize!=0) {
        da->dump[ndump++]=' ';
